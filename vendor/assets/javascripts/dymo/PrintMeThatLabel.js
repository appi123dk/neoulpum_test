        ...
        */
        ...
        printButton.onclick = function()
        {
          var button = $(this).ladda(),
              ...
              //printer variable
              settings.currentPrinterName = printersComboBox.value,
              printer = printers[settings.currentPrinterName],
              label = addressLabel,
              objName = "Address";
              ...
              if (order_number == "") {
                ...
                $.each(menu_array, function(index, menu){
                    var menuSize = menu.size == "1" ? "(Small) " : "(Tall) ",
                        text = "주문번호 : " + order_number + "번" + " \n " + menuSize + menu.name + " \n " + menu.price + " 원",
                        labelSet = new dymo.label.framework.LabelSetBuilder();

                    labelSet.addRecord().setText(objName, text);

                    for(var i=0; i < menu.unit; i++){
